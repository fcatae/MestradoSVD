...
SVDRec svdLAS2A(SMat A, long dimensions) {
  ...
  return svdLAS2(A, dimensions, 5, end, kappa);
}

SVDRec svdLAS2A_iteration(SMat A, long dimensions, long iterations) {
  double end[2] = {-1.0e-30, 1.0e-30};
  double kappa = 1e-6;
  if (!A) {
    svd_error("svdLAS2A called with NULL array\n");
  ...
  return svdLAS2(A, dimensions, iterations, end, kappa);
}
